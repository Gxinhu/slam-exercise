...
void bundleAdjustmentGaussNewton(const VecVector3d& points_3d,
    const VecVector2d& points_2d, const cv::Mat& intrinsic_matrix,
    Sophus::SE3d& pose);
...
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeProjection(Eigen::Vector3d pose, Eigen::Matrix3d K)
        : pose3d_(std::move(pose)), K_(std::move(K)) {}
    ...
    void linearizeOplus() override {
        ...
        _jacobianOplusXi << -fx * inv_z, 0, fx * camera_point[0] * inv_z_square,
            ...
            -fx - fx * pow(camera_point[0], 2) * inv_z_square,
            fx * camera_point[1] * inv_z, 0, -fy * inv_z,
            fy * camera_point[1] * inv_z,
            fy + fy * pow(camera_point[1], 2) * inv_z_square,
            -fy * camera_point[0] * camera_point[1] * inv_z_square,
...
void bundleAdjustmentG2o(const VecVector2d& points_2d,
    const VecVector3d& points_3d, const cv::Mat& K, Sophus::SE3d& pose);
int main() {

    std::string first_file  = "/workspace/slam-exercise/src/ch7/1.png";
    std::string second_file = "/workspace/slam-exercise/src/ch7/2.png";
    std::string first_depth_file =
        "/workspace/slam-exercise/src/ch7/1_depth.png";
    std::string second_depth_file =
        "/workspace/slam-exercise/src/ch7/2_depth.png";
    ...
    for (auto m : matches) {
        ushort d = img_depth_1.ptr<unsigned short>(int(
            keypoints_1[m.queryIdx].pt.y))[int(keypoints_1[m.queryIdx].pt.x)];
        if (d == 0) { // bad depth
            continue;
        }
        float dd = d / 5000.0;
        cv::Point2d p1 =
            pixel2cam(keypoints_1[m.queryIdx].pt, kIntrinsicMatrix);
    ...
    cv::Mat move_vector;
    cv::solvePnP(points_3d, points_2d, kIntrinsicMatrix, cv::Mat(),
        rotation_vector, move_vector);
    ...
    for (int i = 0; i < points_3d.size(); ++i) {
        points_3d_eigen.push_back(
            Eigen::Vector3d(points_3d[i].x, points_3d[i].y, points_3d[i].z));
        points_2d_eigen.push_back(
            Eigen::Vector2d(points_2d[i].x, points_2d[i].y));
    }

    Sophus::SE3d pose;
    bundleAdjustmentGaussNewton(
        points_3d_eigen, points_2d_eigen, kIntrinsicMatrix, pose);
    bundleAdjustmentG2o(
        points_2d_eigen, points_3d_eigen, kIntrinsicMatrix, pose);
    return 0;
}
void bundleAdjustmentGaussNewton(const VecVector3d& points_3d,
    const VecVector2d& points_2d, const cv::Mat& intrinsic_matrix,
    ...
    for (int iter = 0; iter < iteration; ++iter) {
        ...
        for (int i = 0; i < points_3d.size(); ++i) {
            ...
            double inv_z                 = 1.0 / camera_point[2];
            double inv_z_square          = inv_z * inv_z;
            Eigen::Vector2d pixel(fx * camera_point[0] * inv_z + cx,
                fy * camera_point[1] * inv_z + cy);
            Eigen::Vector2d error = points_2d[i] - pixel;
            cost += error.squaredNorm();
            Eigen::Matrix<double, 2, 6> jacobian_matrix;
            jacobian_matrix << -fx * inv_z, 0,
                fx * camera_point[0] * inv_z_square,
                fx * camera_point[0] * camera_point[1] * inv_z_square,
                -fx - fx * pow(camera_point[0], 2) * inv_z_square,
                fx * camera_point[1] * inv_z, 0, -fy * inv_z,
                fy * camera_point[1] * inv_z,
