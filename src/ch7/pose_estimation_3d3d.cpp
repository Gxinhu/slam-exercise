...
void bundleAdjustmentG2o(const std::vector<cv::Point3d>& points_3d1,
    const std::vector<cv::Point3d>& points_3d2, Sophus::SE3d& pose);
void svd_pose_estimation(const std::vector<cv::Point3d>& points_3d1,
    const std::vector<cv::Point3d>& points_3d2, cv::Mat& R, cv::Mat t);
int main() {
    ...
    for (auto m : matches) {
        ushort d1 = img_depth_1.ptr<unsigned short>(int(
            keypoints_1[m.queryIdx].pt.y))[int(keypoints_1[m.queryIdx].pt.x)];
        ushort d2 = img_depth_2.ptr<unsigned short>(int(
            keypoints_2[m.trainIdx].pt.y))[int(keypoints_2[m.trainIdx].pt.x)];
        if (d1 == 0) { // bad depth
            continue;
        }
        if (d2 == 0) { // bad depth
        ...
        cv::Point2d p2 =
            pixel2cam(keypoints_2[m.trainIdx].pt, kIntrinsicMatrix);
...
void svd_pose_estimation(const std::vector<cv::Point3d>& points_3d1,
    ...
    for (int i = 0; i < match_size; ++i) {
        W += Eigen::Vector3d(points_3d1[i].x, points_3d1[i].y, points_3d1[i].z)
           * Eigen::Vector3d(points_3d2[i].x, points_3d2[i].y, points_3d2[i].z)
                 .transpose();
    }
    ...
    Eigen::Vector3d t_eigen =
        Eigen::Vector3d(centroid1.x, centroid1.y, centroid1.z)
...
void bundleAdjustmentG2o(const std::vector<cv::Point3d>& points_3d1,
    ...
    for (size_t i = 0; i < points_3d1.size(); ++i) {
        ...
        Eigen::Vector3d point1_eigen =
            Eigen::Vector3d(point_3d1.x, point_3d1.y, point_3d1.z);
        Eigen::Vector3d point2_eigen =
            Eigen::Vector3d(point_3d2.x, point_3d2.y, point_3d2.z);
